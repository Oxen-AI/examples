...
While these datasets are freely available, they are notoriously difficult to collaborate on for machine learning applications due to a few key factors:
...
These images have two extremely crucial pieces of metadata that cannot be ignored during model development: 
...
In addition to being a more manageable (12GB) dataset size for model fine-tuning and evaluation applications, these human-verified labels will give any downstream models the best possible signal with which to learn to detect fires in novel imagery. ![A synthetic color rendering of a landsat image of a fire zone, showing extensive smoke but partially obscured by cloud cover](images/landsat_burn_plain.png)

![The same image, with a red pixel mask denoting the burn area](images/landsat_burn_annotated.png)
(the image above, overlaid with the researchers' manual annotations of the fire area).
...
**1. Enable atomic changes.** If we want to add, modify, or remove 50 MB of data from this dataset, we should only have to push (and your colleagues pull) 50 MB — not the whole 10 GB zip file. If we want to roll these changes back, we shouldn't have to hunt for the `DATA_VERSION_3_FINAL.zip` taking up extra storage on our drive - we'll leverage Oxen's version control to seamlessly undo the change. 

**2. Improve data-label linking.** The source data and 6 human and computed label sets are currently linked somewhat tenuously through pattern matching in the file names. We'll nail this down to an authoritative data -> labels mapping file to save our teammates tedious, error-prone work and speed up the model development process. 
...
Integration of these changes by other team members would then require a re-download of the full dataset and manual merging with their local work (without automatic awareness of changes made by teammates). 

Instead, we can unpack this data into an Oxen repository so that all our teammates can easily: 
- Examine, query and explore the data in plain text on 
- Clone it with `oxen clone https://www.hub.oxen.ai/ba/ActiveFire`
- Create a space-saving local working copy with `oxen checkout -b my-local-branch`
- Contribute to the improvement of the repo through adding, deleting, and modifying additional files without needing to re-package and re-push all 10GBs of data. 

**Early exploration and understanding**
While waiting for the .zip files to download and unpack from the Google Drive source, we created a README.

Included with the data was a tabular file titled `images202009.csv`—we couldn't quite tell what this file represented, so we pushed it up to oxen as well to explore further. 
...
The `productId` column ("LC08") and `cloudCover` tipped us off that this is a metadata export of full 5600x6100 pixel LANDSAT scenes from which the researchers exported their 256x256 pixel patches. 
...
We can use the natural language query interface to translate our analytic questions into SQL for hassle-free understanding which terrain correction methods are present in the dataset - the imbalance here between Precision and Terrain Correction (L1TP) and Systematic Terrain Correction (L1GT) is important to flag for our team during model and data development, as they could cause inconsistencies in the resulting data. 
...
...so solidifying awareness of coverage across the various labeling types in one place is key here. 
